...
impl App {
    fn new(engine: LsmEngine) -> Self {
        ...
        log.push_back(("ApexStore TUI Dashboard \u{2014} engine ready.".into(), C_AMBER));
        ...
        for _ in 0..24 { ops_history.push_back(0u64); }
        Self {
            engine,
            focus: Focus::Input,
            input: Input::default(),
        ...
        match parts[0].to_uppercase().as_str() {
            ...
            "SET" => {
                ...
                match self.engine.set(key.clone(), val) {
                    Ok(_)  => { self.log_push(format!("\u{2713} SET '{}' OK", key), C_OK); self.incr_ops(); }
            ...
            "GET" => {
                ...
                match self.engine.get(parts[1]) {
                    Ok(Some(v)) => {
                        self.log_push(format!("\u{2713} '{}' = '{}'", parts[1], String::from_utf8_lossy(&v)), C_OK);
                        self.incr_ops();
                    }
                    Ok(None)   => self.log_push(format!("\u{26a0}  Key '{}' not found", parts[1]), C_WARN),
                    Err(e)     => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            }
            ...
            "DEL" | "DELETE" => {
                ...
                match self.engine.delete(key.clone()) {
                    Ok(_)  => { self.log_push(format!("\u{2713} DEL '{}' (tombstone written)", key), C_OK); self.incr_ops(); }
                    Err(e) => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            }
            ...
            "SEARCH" => {
                ...
                let result      = if prefix_mode { self.engine.search_prefix(query) }
                                  else           { self.engine.search(query) };
                match result {
                    Ok(rows) if rows.is_empty() => self.log_push("\u{26a0}  No records found", C_WARN),
                    Ok(rows) => {
                        ...
                        if rows.len() > 20 { self.log_push(format!("  ... and {} more", rows.len()-20), C_DIM); }
                        self.incr_ops();
                    }
                    Err(e) => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            ...
            "SCAN" => {
                ...
                match self.engine.search_prefix(parts[1]) {
                    Ok(rows) if rows.is_empty() => self.log_push(format!("\u{26a0}  No records with prefix '{}'", parts[1]), C_WARN),
                    Ok(rows) => {
                        self.log_push(format!("\u{2713} {} record(s) [prefix='{}']:", rows.len(), parts[1]), C_OK);
                        for (k, v) in rows.iter().take(20) {
                            self.log_push(format!("  {} = {}", k, String::from_utf8_lossy(v)), C_TEXT);
                        }
                        if rows.len() > 20 { self.log_push(format!("  ... and {} more", rows.len()-20), C_DIM); }
            ...
            "ALL" => {
                match self.engine.scan() {
                    ...
                    Ok(rows) => {
                        ...
                        if rows.len() > 30 { self.log_push(format!("  ... and {} more", rows.len()-30), C_DIM); }
                        self.incr_ops();
                    }
                    Err(e) => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            ...
            "KEYS" => {
                match self.engine.keys() {
                    ...
                    Ok(keys) => {
                        ...
                        if keys.len() > 30 { self.log_push(format!("  ... and {} more", keys.len()-30), C_DIM); }
                        self.incr_ops();
                    }
                    Err(e) => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            ...
            "COUNT" => {
                match self.engine.count() {
                    Ok(n)  => { self.log_push(format!("\u{2713} Total active records: {}", n), C_OK); self.incr_ops(); }
                    Err(e) => self.log_push(format!("\u{274c} {}", e), C_ERR),
                }
            }
                ...
                if parts.len() >= 3 && parts[1].to_uppercase() == "SET" {
                    ...
                    match std::fs::read_to_string(file_path) {
                        Ok(content) => {
                            ...
                            for (line_no, line) in content.lines().enumerate() {
                                ...
                                if let Some((k, v)) = line.split_once('=') {
                                    match self.engine.set(k.trim().to_string(), v.trim().as_bytes().to_vec()) {
                                        ...
                                        Err(e) => { self.log_push(format!("  line {}: {}", line_no+1, e), C_ERR); err += 1; }
                                    }
                                } else {
                            ...
                            self.log_push(format!("\u{2713} {} imported, {} errors  [{:.1?}]", ok, err, t.elapsed()), C_OK);
                        }
                        Err(e) => self.log_push(format!("\u{274c} Cannot read '{}': {}", file_path, e), C_ERR),
                    }
                } else if parts.len() >= 2 {
                    match parts[1].parse::<usize>() {
                        Ok(n) => {
                            ...
                            let mut errs = 0usize;
                            for i in 0..n {
                                match self.engine.set(format!("batch:{:06}", i), format!("value_{}", i).into_bytes()) {
                                    Ok(_)  => { self.incr_ops(); }
                                    Err(_) => { errs += 1; }
                                }
                            }
                            ...
                            self.log_push(format!("\u{2713} {} records in {:.2?}  ({:.0} ops/s)  errors={}", n, elapsed, rate, errs), C_OK);
                        }
            ...
            // DEMO ─────────────────────────────────────────────────────────────
            "DEMO" => {
                self.log_push("\u{2500}\u{2500}\u{2500} Running Demo \u{2500}\u{2500}\u{2500}".to_string(), C_ORANGE);
                let t = Instant::now();
                for i in 0..100 {
                    let _ = self.engine.set(format!("demo:{:04}", i), format!("demo-value-{}", i).into_bytes());
                    self.incr_ops();
                }
                ...
                self.log_push(format!("\u{2713} Demo done in {:.2?}  active keys={}", t.elapsed(), count), C_OK);
            }
            ...
            unknown => {
                self.log_push(format!("\u{274c} Unknown command '{}'. Type HELP.", unknown), C_ERR);
            }
        }
    }
}
...
fn render_title(f: &mut Frame, area: Rect, app: &App) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            Span::styled(" \u{26a1} ",       Style::default().fg(C_ORANGE).add_modifier(Modifier::BOLD)),
        ...
        .title_bottom(Line::from(vec![
            ...
            Span::styled(format!("{}", total),         Style::default().fg(C_OK).add_modifier(Modifier::BOLD)),
            Span::styled("  ops/s: ",    Style::default().fg(C_DIM)),
            Span::styled(format!("{:.0}", app.ops_per_sec), Style::default().fg(C_ORANGE).add_modifier(Modifier::BOLD)),
...
fn render_stats(f: &mut Frame, area: Rect, app: &App) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            ...
            Span::styled("LSM-Tree Statistics ", Style::default().fg(C_AMBER).add_modifier(Modifier::BOLD)),
        ]));
    ...
    f.render_widget(
        Gauge::default()
            .block(Block::default()
                .title(Span::styled(" MemTable ", Style::default().fg(C_TEXT).add_modifier(Modifier::BOLD)))
                .borders(Borders::LEFT | Borders::RIGHT)
                .border_style(Style::default().fg(C_BORDER)))
            .gauge_style(Style::default().fg(pct_color((mem_pct*100.0) as u8))
                .bg(Color::Rgb(25, 30, 50)).add_modifier(Modifier::BOLD))
            .ratio(mem_pct)
            .label(Span::styled(
    ...
    f.render_widget(
        Gauge::default()
            .block(Block::default()
                .title(Span::styled(" SSTables ", Style::default().fg(C_TEXT).add_modifier(Modifier::BOLD)))
                .borders(Borders::LEFT | Borders::RIGHT)
                .border_style(Style::default().fg(C_BORDER)))
            .gauge_style(Style::default().fg(pct_color((sst_ratio*100.0) as u8))
                .bg(Color::Rgb(25, 30, 50)).add_modifier(Modifier::BOLD))
            .ratio(sst_ratio)
            .label(Span::styled(
    ...
    f.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(format!("{:.1}", app.ops_per_sec), Style::default().fg(C_ORANGE).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
...
fn render_clock(f: &mut Frame, area: Rect, _app: &App) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            ...
            Span::styled("System Clock ", Style::default().fg(C_CLOCK).add_modifier(Modifier::BOLD)),
        ]));
    ...
    f.render_widget(
        Paragraph::new(date_str).alignment(Alignment::Center)
...
fn render_log(f: &mut Frame, area: Rect, app: &App) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            ...
            Span::styled("Command Log ",    Style::default().fg(C_TEXT).add_modifier(Modifier::BOLD)),
            Span::styled(format!("({} lines) ", app.log.len()), Style::default().fg(C_DIM)),
        ]));
    ...
    let items: Vec<ListItem> = app.log.iter()
        .rev().take(inner_h).rev()
        .map(|(msg, col)| ListItem::new(Line::from(
...
fn render_input(f: &mut Frame, area: Rect, app: &App) {
    ...
    let block = Block::default()
        ...
        .title(Line::from(vec![
            ...
            Span::styled("Command Input ",
                Style::default().fg(if focused { C_ORANGE } else { C_DIM }).add_modifier(Modifier::BOLD)),
            Span::styled(if focused { "(active) " } else { "(Tab to focus) " },
                Style::default().fg(C_DIM).add_modifier(Modifier::ITALIC)),
        ]))
...
fn render_statusbar(f: &mut Frame, area: Rect, app: &App) {
    ...
    let bar = Paragraph::new(Line::from(vec![
        ...
        Span::styled("| ", Style::default().fg(C_BORDER)),
        Span::styled(format!(" mouse ({},{}) ", app.mouse_pos.0, app.mouse_pos.1), Style::default().fg(C_DIM)),
