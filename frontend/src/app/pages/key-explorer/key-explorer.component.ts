      ...
      <div class="page-header">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button class="btn btn-secondary" (click)="loadAllKeys()" [disabled]="loadingList()">
            ...
            @else { <hugeicons-icon [icon]="ListViewIcon" [size]="16" [strokeWidth]="1.5" /> }
            List All Keys
          </button>
          <button class="btn btn-secondary" (click)="runScan()" [disabled]="loadingScan()">
            @if (loadingScan()) { <span class="spinner"></span> }
            @else { <hugeicons-icon [icon]="SearchIcon" [size]="16" [strokeWidth]="1.5" /> }
            Full Scan
          </button>
        </div>
      </div>
          ...
          <div class="card-body">
            <div class="row-inline">
              ...
              <button class="btn btn-success" style="align-self:flex-end" [disabled]="!lookupKey.trim()||loadingLookup()" (click)="lookup()">
                ...
                @else { <hugeicons-icon [icon]="Search01Icon" [size]="16" [strokeWidth]="1.5" /> }
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="card-body">
            <div class="row-inline">
              ...
              <button class="btn btn-primary" style="align-self:flex-end" [disabled]="!insertKey.trim()||!insertValue.trim()||loadingInsert()" (click)="insert()">
                ...
                @else { <hugeicons-icon [icon]="PencilEdit01Icon" [size]="16" [strokeWidth]="1.5" /> }
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <button class="btn btn-primary" style="margin-top:10px;width:100%;justify-content:center" [disabled]="!batchJson.trim()||loadingBatch()" (click)="runBatch()">
              ...
              @else { <hugeicons-icon [icon]="PackageIcon" [size]="16" [strokeWidth]="1.5" /> }
              Insert Batch
            </button>
          </div>
        </div>
        ...
        @if (entries().length === 0) {
          ...
          <div class="table-wrapper">
            <table class="kv-table">
              <thead>
                <tr><th>Key</th><th>Value</th><th>Time</th><th>Actions</th></tr>
              </thead>
              <tbody>
                @for (e of filteredEntries(); track e.key) {
                  <tr>
                    ...
                    <td class="actions-cell">
                      <button class="btn btn-secondary btn-sm icon-btn" (click)="refetch(e.key)" title="Refetch">
                        <hugeicons-icon [icon]="RefreshIcon" [size]="14" [strokeWidth]="1.5" />
                      </button>
                      <button class="btn btn-danger btn-sm icon-btn" (click)="deleteKey(e.key)" title="Delete">
                        <hugeicons-icon [icon]="Delete01Icon" [size]="14" [strokeWidth]="1.5" />
                      </button>
                    </td>
                  </tr>
                }
...
export class KeyExplorerComponent {
  ...
  lookupKey = ''; insertKey = ''; insertValue = '';
  searchQ = ''; searchPrefix = false; batchJson = ''; filterText = '';
  ...
  lookup(): void {
    const key = this.lookupKey.trim(); if (!key) return;
  ...
  runSearch(): void {
    ...
    this.store.search(q, this.searchPrefix).subscribe({
      next: (records) => { records.forEach(r => this.upsert(r.key, r.value)); this.toast.success(`${records.length} keys found`); this.viewMode.set('table'); this.loadingSearch.set(false); },
  ...
  runBatch(): void {
    ...
    this.store.setBatch(records).subscribe({
      ...
      error: (e) => { this.toast.error(e?.error?.message ?? 'Batch failed'); this.loadingBatch.set(false); }
    });
  }
  ...
  loadAllKeys(): void {
    ...
    this.store.listKeys().subscribe({
      next: (keys) => {
        keys.forEach(k => { if (!this.entries().find(e => e.key === k)) this.entries.update(l => [...l, { key: k, value: '—', fetchedAt: new Date() }]); });
        this.toast.success(`${keys.length} keys listed`); this.viewMode.set('table'); this.loadingList.set(false);
      },
  ...
  runScan(): void {
    ...
    this.store.scan().subscribe({
      next: (records) => { this.entries.set(records.map(r => ({ key: r.key, value: r.value, fetchedAt: new Date() }))); this.toast.success(`${records.length} records loaded`); this.viewMode.set('scan'); this.loadingScan.set(false); },
  ...
  deleteKey(key: string): void {
    this.store.delete(key).subscribe({
      next: () => { this.entries.update(l => l.filter(e => e.key !== key)); this.toast.success(`Key "${key}" deleted`); },
