    ...
    <div class="page">
      ...
      <div class="card" style="margin-bottom:24px">
        <div class="card-header">
          <span class="op-badge post">POST</span>
          ...
          <div class="form-row">
            ...
            <div style="display:flex;gap:8px;align-self:flex-end">
              <button class="btn btn-success" [disabled]="!newName.trim()||loadingNew()" (click)="createFlag(true)">
                @if (loadingNew()) { <span class="spinner"></span> } @else { ✅ }
                Enable
              </button>
          ...
          @for (flag of flags(); track flag.name) {
            <div class="flag-card" [class.flag-enabled]="flag.enabled" [class.flag-disabled]="!flag.enabled">
              ...
              <div class="flag-actions">
                @if (!flag.enabled) {
                  <button class="btn btn-success btn-sm" (click)="toggle(flag, true)">✅ Enable</button>
                } @else {
                  <button class="btn btn-danger btn-sm" (click)="toggle(flag, false)">⛔ Disable</button>
                }
              </div>
            </div>
          }
