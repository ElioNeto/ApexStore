...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="card" style="margin-bottom:24px">
        <div class="card-header">🔑 <span class="card-title">Bearer Token da Sessão</span></div>
        <div class="card-body">
          ...
          <div class="row-inline">
            ...
            <button class="btn btn-primary" style="align-self:flex-end" (click)="saveSessionToken()">Salvar</button>
            @if (hasSessionToken()) {
              <button class="btn btn-danger" style="align-self:flex-end" (click)="clearSessionToken()">Limpar</button>
            }
          </div>
          @if (hasSessionToken()) {
            <div class="token-active-badge">✅ Token ativo na sessão</div>
          }
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:24px">
        ...
        <div class="card-body">
          ...
          <button
            ...
            [disabled]="!newName.trim() || newPerms().length === 0 || loadingCreate()"
            (click)="createToken()"
          >
            @if (loadingCreate()) { <span class="spinner"></span> } @else { ➕ }
            Create Token
          </button>

          @if (createdToken()) {
            <div class="created-token-box">
              <div class="created-token-label">⚠️ Copie agora — este token não será exibido novamente</div>
              <div class="created-token-value">{{ createdToken() }}</div>
              <button class="btn btn-secondary btn-sm" (click)="copyToken()">📋 Copy</button>
            </div>
          }
        </div>
      </div>
              ...
              <tbody>
                @for (t of tokens(); track t.id) {
                  <tr [class.expired]="isExpired(t)">
                    ...
                    <td>
                      <button class="btn btn-danger btn-sm" (click)="revokeToken(t.id)">🗑️ Revoke</button>
                    </td>
                  </tr>
                }
              </tbody>
  ...
  styles: [`
    ...
    .perm-toggle {
      ...
      input { display:none; }
      &.selected { border-color:var(--accent); background:var(--accent-dim); color:var(--accent); }
    }
    .token-active-badge { margin-top:10px; font-size:0.82rem; color:var(--green); }
    .created-token-box {
      margin-top:16px; background:var(--bg-primary);
      border:1px solid rgba(249,115,22,0.4); border-radius:10px; padding:16px;
      display:flex; flex-direction:column; gap:10px;
    }
    .created-token-label { font-size:0.82rem; color:var(--accent); font-weight:600; }
    ...
    .auth-error {
      ...
      background:var(--red-dim); margin:0;
    ...
    .perm-read   { background:var(--blue-dim); color:var(--blue); }
    .perm-write  { background:var(--accent-dim); color:var(--accent); }
    .perm-delete { background:var(--red-dim); color:var(--red); }
    .perm-admin  { background:rgba(168,85,247,0.12); color:var(--purple); }
  `]
})
