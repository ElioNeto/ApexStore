...
3. Após cada bloco: `npx tsx scripts/check-todos.ts .task-state.json` → se falhar, corrigir e repetir (máx 3x por TODO).
4. Ao concluir todos: `npx tsx scripts/workflow-agent.ts .github/workflows/ci.yml` → se falhar, acionar @debugger inline com o JSON de saída.
